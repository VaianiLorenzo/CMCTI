...
def train_model(cfg, model, device, n_epochs, optimizer, scheduler, train_dataloader, val_dataloader,
    ...
    for epoch in range(0, n_epochs):
        ...
        ground_truth = []
        print("init. training")
        for _, data in enumerate(tqdm(train_dataloader), 0):
...
def train_multitask_model(cfg, model, device, n_epochs, optimizer, scheduler, train_dataloader, val_dataloader,
                path_dir_checkpoint, comet_exp):
    ...
    for epoch in range(0, n_epochs):
        ...
        for _, data in enumerate(tqdm(train_dataloader), 0):
            ...
            # update lists for accuracy computation
            binary_out = [o[0].item() for o in outputs]
            source_modality_out = [o[1].item() for o in outputs]
            list_binary_outputs.extend(list(binary_out))
            list_source_modality_outputs.extend(list(source_modality_out))
            
            binary_tar = [t.item() for t in binary_targets]
            binary_ground_truth.extend(binary_tar)

            source_modality_tar = [t.item() for t in source_modality_targets]
            source_modality_ground_truth.extend(source_modality_tar)

            # compute loss
            binary_loss = binary_loss_function([o[0] for o in outputs], binary_targets)
            source_modality_loss = source_modality_loss_function([o[1] for o in outputs], source_modality_targets)
